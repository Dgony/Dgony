...
4. 클래스는 정의된 속성(필드)과 메서드(함수)를 가진다. 클래스로 만드는 인스턴스(객체)는 클래스의 속성과 메서드를 가진다고 말할 수 있다.
...
3. DOM tree라는 표현은 주로 HTML이나 XML 구조를 표현하는데 사용된다.   
4. DOM은 HTML, XML 문서의 프로그래밍 인터페이스이다. 문서의 구조화된 표현을 제공하여 프로그래밍 언어가 접근하고 변경할 수 있도록 한다.  
프로그래밍 인터페이스를 동해 프로그래머가 웹 페이지의 구조와 내용을 동적으로 조작할 수 있게 한다.
5. 'DOM을 사용한다'는 JS만 가능하지만, python이나 java도 JS와는 다른방법으로 DOM에 접근하여 조작할 수 있다.  
