...
1. 네트워크의 최대 전송 단위(MTU, Maximum Transmission Unit) 제한이 있기 때문   
    * 대부분의 네트워크의 MTU는 1500 바이트  
    * MTU를 넘는 데이터를 보내기 위해 패킷 단위로 분할  
2. 에러 해결 용이 - 에러가 발생한 해당 패킷만 재전송하면 해결  
3. 네트워크 혼잡 해결 - 각 패킷은 다른 경로 활용 가능, 여러 사용자가 네트워크 동시 활용 가능  
4. TCP - 패킷 단위로 데이터 제어(흐름 및 속도 제어 등)  
2. 패킷을 보내는 과정   
ex)웹페이지 방문 시, 웹페이지 쪽 장치가 데이터(텍스트, 이미지)를 여러 개의 Packet으로 나누어 네트워크(인터넷, LAN)를 통해 중간장비(라우터, 스위치)를 거쳐 목적지까지 보내게 됨.   


### Router(라우터, 공유기)  
* **여러 네트워크(가정용 네트워크, 인터넷 등)를 연결해주는 장치로, 데이터를 올바른 목적지로 전달하는 역할**  
1. 네트워크 연결 - 서로 다른 네트워크 연결  
...
### OSI 7계층
**OSI 7계층이란 국제 표준화 기구인 ISO(Internatioal Standardization Organization)  
에서 개발한 컴퓨터 네트워크 프로토콜 디자인과 통신을 계층으로 나누어 설명한 개방형 시스템 상호 연결 모델** 

1. 물리 계층(Physical Layer)  
    * 하드웨어에서 0과 1의 비트 정보를 회선에 보내기 위한 전기적 신호 변환
    * LAN 케이블, 전파 

2. 데이터링크 계층(DataLink Layer)
    * 인접 시스템 간 데이터 전송, 전송 오류 제어 / 동기화, 오류 제어, 흐름 제어, 회선 제어
    * Ethernet(얘도 프로토콜의 일종), 스위치(MAC 주소 기반, 같은 네트워크에서 장치들끼리 데이터 전달)

3. 네트워크 계층(Network Layer)
    * 단말기 간 데이터를 전송하기 위한 최적화된 경로 제공
    * IP, 라우터(서로 다른 네트워크간 데이터 전달)  

4. 전송 계층(Transport Layer)
    * 송수신 프로세스 간의 연결 / 신뢰성 있는 통신 보장 / 데이터 분할, 제조립, 흐름제어, 오류 제어, 혼잡 제어
    * TCP, UDP  
...
* 통신이 일어날 때 1 ~ 7 혹은 7 ~ 1 순서로 흐름  
    * 데이터를 보낼 떄 ==> 7 ~ 1로 내려가면서 encapsulation
    * 데이터를 받을 때 ==> 1 ~ 7로 올라가면서 decapsulation

* **상위 계층을 사용한다는 것은, 하위 계층도 함께 사용한다는 의미** 

* 📡 HTTP 웹사이트 접속 (예: http://example.com)      
    * HTTP 요청 생성 (7계층)

    * SSL/TLS 암호화 처리 (6계층, HTTPS인 경우)

    * 세션 유지 처리 (5계층)

    * TCP 포트 80/443 연결 (4계층)
...
### TCP/IP 4계층  
* **더 실용적으로 쓰이는 모델로, OSI보다 계층이 간략화되어 있음  
또한 프로토콜로 쓰이는 TCP와 다르게, TCP/IP는 OSI와 같은 계층 모델**  

* TCP/IP(Transmission Control Protocol / Internet Protocol)  
==> 계층모델도 프로토콜과 같은 약자 사용

* 오늘날 **전체 네트워크 통신 체계(모델)** 를 지칭하는 말로도 쓰임  
...
IP를 제공받는 개인이나 기업은 인터넷과 직접 연결되어 있는것이 아니며, ISP를 경유해서 연결되어 있음    
ISP는 Backbone Network(백본망) 혹은 IX(Internet Exchange)등을 통해 다른 ISP 혹은 글로벌 네트워크에 연결    
개인이 인터넷과 직접 연결되려면 자체적으론 ISP와 같은 백본망 등을 구축해야 함 ==> 불가능  
```
[개인 or 기업의 장비]   
   ↓  
[라우터 or 모뎀]  
...
[ISP 장비 (통신사)]  
   ↓  
[전세계 인터넷망] ==> 여러 ISP, 대형 데이터 센터, CDN, 클라우드, 해저 케이블망 등이 연결된 거대한 네트워크 
```
...
4. Protocol - 통신을 하기 위한 약속, 패킷을 전달할 때 지켜야 할 규칙(TCP, IP) 
...
9. Packet, 패킷 - 네트워크를 통해 전달되는 데이터의 작은 조각     
ex)웹페이지를 방문할 때, 보내는 쪽 장치가 웹페이지의 데이터(이미지, 텍스트)를 여러 개의 Packet으로 나누어 네트워크(인터넷, LAN)를 통해 중간장비(라우터, 스위치)를 거쳐 목적지까지 전달한다.   
10. MTU(Maximum Transmission Unit, 최대 전송 단위) 제한   
대부분의 네트워크는 1500 바이트의 MTI를 가지므로, 이 제한을 넘는 데이터를 보내기 위해 패킷 사용   
11. TCP(Transmisson Control Protocol) - 네트워크 연결 방식 중 하나, 후술  
12. UDP(User Diagram Protocol) - 네트워크 연결 방식 중 하나, 후술
...
* 실습 가능 항목  
    ...
    * 장비 간의 케이블 연결 (직렬, 크로스, 콘솔 등)
    * IP 주소 설정, 라우팅 프로토콜 구성 (RIP, OSPF 등)
    * VLAN, NAT, DHCP, DNS, ACL 실습
    * 패킷 캡처 및 분석 (시뮬레이션 모드)
...
### 소규모 기업, 사무실 네트워크 구축  
기업과 ISP, 웹서버의 유무선 연결을 보여주는 네트워크 구축   

* 내가 표현하고 싶은 주 기능  
    * 먼 거리의 Office와 Data Center(db 혹은 웹서버 등)와 연결해 네트워크 흐름 확인  
    * 라우터 아래 같은 네트워크를 공유하는 단말 연결  
    * 단말기 무선 연결   
    * 서로 다른 네트워크를 가진 라우터간 연결을 통한 다른 네트워크 단말기 간의 통신  
    * 통신 확인은 ICMP, HTTP 등을 통해 테스트 
    * DNS 서버도 추가  

* 일단정리   
Core Router - 백본망 구축 라우터, BGP를 이용해 다른 ISP와 경로 교환, 코어라우터끼리, 글로벌 백본과 함께 등  
Edge Router - ISP의 경계 지점에 위치하는 라우터로, 외부 네트워크와 내부 네트워크간 중계 peering, transit    
ISP가 공인 IP를 보내면 Edge Router가 받아서 처리, NAT 기능이 있는 공유기, 방화벽, 서버 등도 가능    
공유기 - Router의 많은 기능 중 NAT만 특화시켜 양산한 기기  
공인 IP는 'DHCP 서버' 혹은 'ISP의 Router'가 보냄  
[ISP] ⇄ [모뎀 or 광랜 단말기] ⇄ [공유기] ⇄ [내부 PC, 스마트폰 등]  
Access ping - AP는 유선 LAN(예: 스위치, 공유기 등)에 연결, 무선 신호(Wi-Fi)로 클라이언트와 통신    
최상위 기관: IANA (Internet Assigned Numbers Authority) = 전 세계의 공인 IP 주소 자원을 관리  
IPv4: 약 43억 개 주소     
IPv6: 사실상 무한대   
IANA는 5개의 RIR(Regional Internet Registry)에 할당, 아시아 = APNIC ==> 한국, 일본, 중국 등       
RIR → ISP에게 준 주소 범위 = IP 풀   
인터넷 상에서 고유해야 하며, 중복되면 안되기에 최상위 기관에서 관리   
이 IP를 통해 글로벌 네트워크와 연결시켜주는 사설 IP를 ISP DHCP 서버에서 발생시켜 클라이언트에게 배포   
packet tracer는 실제 공인 IP를 받는것이 아니기 때문에, 이미 받았다 가정하고 DHCP에서 사설 IP 발생  
사설 IP 대역  
10.0.0.0 ~ 10.255.255.255  
172.16.0.0 ~ 172.31.255.255  
192.168.0.0 ~ 192.168.255.255  
DHCP 기능	사설 IP(예: 192.168.x.x)를 클라이언트에게 자동으로 할당해주는 기능  
클라이언트가 DHCP 서버에 연결되면 자동으로 사설 IP를 받고 통신 가능   
ISP에서 DHCP용 서버를 만들고 서비스 - DHCP 탭에서 다음과 같이 설정  
Default Gateway:  192.168.10.1  
DNS Server:       8.8.8.8 (또는 아무 IP)  
Start IP Address: 192.168.10.10  
Subnet Mask:      255.255.255.0  
Maximum Number:   50(최대 클라이언트 수)      
이후 스위치나 라우터 설정을 위해 console 케이블로 PC와 연결, 터미널로 접근 후 다음 설정  
enable  
conf t  
interface gig0/0  
...
no shutdown  
exit   
라우터 - OSI 3계층인 네트워크 계층에서 사용, IP 주소기반, 네트워크 간 연결  
스위치 - OSI 2계층인 데이터링크 계층에서 사용, MAC 주소기반, LAN 내부에서 장비간 연결   
라우터도 스위치처럼 동작할 수는 있지만, 필요 이상의 기능, 보안 검사, 경로 계산 등 때문에 느려지거나 과부화 됨  
스위치는 많은 장비가 한 네트워크에서 빠르게 통신할 수 있게 만들어졌기 때문에, 스위치가 필수    
그래서 서버 1대 단독구성인 경우, 스위치는 필요가 없는 경우가 생길 수도 있음    
DHCP에서 라우터로 보내주는 IP주소는 DHCP를 처음 설정할 때 default gateway와 같게 설정해야 한다.  
서브넷 마스크는 기본값인 255.255.255.0  
라우터의 기본 설정을 위해 config의 static 항목에서 network / mask / next hop 설정 가능   
network 신호가 오면 next hop으로 가도록 지정 가능   
dhcp 서버에 1,2,3 케이블이 연결되어 있을 때, 케이블 별로 ip configuration을 통해 dhcp인지, static address인지 결정 가능     
라우터의 static routes - 0.0.0.0/0 via 209.165.201.1 ==> 경로를 모를 땐, 209.165.201.1 로 보내라    
라우터에서 ip route 192.168.10.0 255.255.255.0 192.168.20.1 같은 static route를 등록하면, 라우터는 다음과 같이 판단합니다:  
"목적지 IP가 192.168.10.0/24 대역이면, 192.168.20.1(다음 라우터)로 보내자."   
한편, 스위치는 IP주소는 신경 안 씁니다.  
라우터가 경로를 결정해서 패킷을 보내면, 그 패킷은 이더넷 프레임(MAC 주소) 형태로 스위치를 지나가게 됩니다.  
스위치는 다음처럼 동작합니다:  
"프레임의 목적지 MAC 주소가 00:1A:2B:3C:4D:5E네? 그건 포트 3에 있어. 그쪽으로 보내!"  
PC1이 192.168.10.5로 패킷을 보냄.  
라우터는 192.168.10.0/24로 가는 static route를 확인하고, 다음 홉으로 결정된 IP로 보냄.  
그 패킷은 MAC 주소로 싸여서 이더넷 프레임이 됨.  
스위치는 MAC 주소 테이블 보고 알맞은 포트로 전달.  
서버 컴퓨터 기준, 1번 케이블 ip configuration - 이 케이블로 연결된 통신에서 나는 이 IP주소를 가진다   
스위치에서 다른 MAC 주소만 가지면 알아서 찾아갈 수 있음. MAC 주소가 같은 DHCP 할당 못 받음   
```
| 구분         | WAN (Wide Area Network)   | LAN (Local Area Network) | VLAN (Virtual LAN)      |
| ---------- | ------------------------- | ------------------------ | ----------------------- |
| 🌍 **의미**  | 광역 네트워크 (지리적으로 넓은 범위)     | 근거리 네트워크 (같은 건물/사무실)     | 가상 LAN (논리적으로 분리된 LAN)  |
| 🗺 **범위**  | 도시, 국가, 세계                | 한 건물, 한 층                | LAN 내부에서 논리적 구분         |
| 🧩 **역할**  | 서로 다른 LAN 연결 (예: 회사 ↔ 본사) | 장비 간 빠른 통신 (PC, 프린터 등)   | 같은 스위치 내에서 네트워크 분리      |
| 🔌 **장비**  | 라우터, 모뎀, 위성 등             | 스위치, 허브                  | VLAN 지원 스위치             |
| 🔐 **보안성** | 낮은 편 (인터넷 포함)             | 상대적으로 높음                 | 매우 높음 (VLAN 간 통신 차단 가능) |
| 🏷 **예시**  | 인터넷, 본사 ↔ 지사 연결           | 사무실 PC 간 네트워크            | 개발팀 VLAN, 인사팀 VLAN      |
```
같은 라우터 내부에서 서로 다른 인터페이스에 동일한 네트워크 대역을 설정하면 안 된다  
즉, 두 포트 모두 같은 네트워크 대역(10.0.0.0/24) 에 속하므로, 라우터 입장에서는 혼란이 생깁니다.  
라우터는 각 인터페이스가 서로 다른 네트워크에 있어야 한다는 원칙을 갖고 있기 때문입니다.  
```
VLAN 생성 및 포트 할당(스위치 CLO)  
