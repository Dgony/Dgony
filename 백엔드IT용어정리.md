...
* 애자일 - 바퀴모양 무한굴레, 일정 주기를 반복하며 개발, 고객의 요구사항에 유연하게 대응 
    * 개인과 소통 > 절차와 도구
    * **스크럼, XP, 칸반, Lean, 크리스탈, ASD, FDD, DSDM, DAD**등이 있음
...
* 모듈
    ...
    * 결합도
        * 자료(data) < 스탬프 < 제어 < 외부 < 공통 < 내용(content), **자스제외공내**
...
### 데이터베이스 설계
* 고려 사항
    * 무결성 - 연ㅅ나 후에도 데이터가 정해진 제약 조건을 항상 만족해야 한다(pk, fk의 무결성을 생각하면 될듯)
    * 일관성
    * 회복
    * 보안
    * 효율성
...
* 화이트박스 테스트 - 기초 경로 검사, 제어 구조 검사(조건, 루프, 데이터 흐름, 화이트기제조루데이트)
...
* IPv4 - unicast / multicast / brodadcast + 64byte 가 최대로 제한
...
* OSI-7 계층
    * 응용계층 - 사용자가 OSI에 접근할 수 있계 응용프로그램 지공
...
* DFD(Data Flow Diagram) - 자료흐름도, Data Store == 평행선
