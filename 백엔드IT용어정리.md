...
## 요구 사항
- 사용자는 이메일과 비밀번호를 입력할 수 있어야 합니다.
- 올바른 이메일과 비밀번호를 입력하면 로그인에 성공하고, 토큰을 발급받습니다.
- 잘못된 이메일이나 비밀번호를 입력하면 에러 메시지를 표시합니다.
...
- POST /api/login
  ...
  - 응답: { token: string } 또는 { error: string }
...
## 비즈니스 로직
- 사용자가 이메일과 비밀번호를 입력하고 로그인 버튼을 클릭하면 /api/login 엔드포인트로 POST 요청을 보냅니다.
- 서버는 이메일과 비밀번호를 확인하고, 일치하면 JWT 토큰을 발급하여 응답합니다.
- 클라이언트는 받은 토큰을 저장하고, 이후 요청 시 인증 헤더에 포함하여 전송합니다.

## 에러 처리
- 이메일 또는 비밀번호가 잘못된 경우 401 에러를 반환하고, "Invalid email or password" 메시지를 표시합니다.

## 테스트 케이스
- 올바른 이메일과 비밀번호로 로그인 시도 시 성공하고, 토큰을 발급받습니다.
- 잘못된 이메일 또는 비밀번호로 로그인 시도 시 401 에러가 반환됩니다.
```
