...
### 데이터 타입의 형변환(promotion)
1. 자동 형변환(묵시적 형변환) - 작은 데이터 타입의 변수가 큰 데이터 타입에 들어갈 때 사용
2. 강제 형변환(명시적 형변환) - 큰 데이터 타입이 작은 데이터 타입에 들어갈 때 사용
3. 데이터 타입을 변환할 대 명시하는지 여부에 따른 구분 
4. 데이터 타입의 크기 비교  
...
### JAVA의 Class  
특정 종류의 인스턴스가 가지는 변수와 메서드를 정의하는 틀(template)을 의미한다.  
택시, 트럭, 버스같은 '인스턴스'들은 차라는 '클래스'의 공동된 특징을 가지는 탈것이다.    
예를들면 '차'라는 클래스는 '굴러간다', '깜박이를 킨다'등의 메서드와 '바퀴', '전조등' 같은 변수를 가진다.  
...
4. 클래스는 정의된 속성(필드)과 메서드(함수)를 가진다. 클래스로 만드는 인스턴스(객체의 복사본)는 클래스의 속성과 메서드를 가진다고 말할 수 있다.
...
부품 조립식 언어로, 반대되는 개념으로 절차지향적 언어가 있다.  
하드웨어의 발전 방식에서 유래되었다.  
