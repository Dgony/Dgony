...
5. 위 사진은 JAVA 프로그램이 실행될 때 RAM의 구조이다.  
실행 클래스(main메서드 포함)에서 프로그램이 처음 실행되면 원본영역(method, static 영역)에 class를 가져온다.  
스택 영역에 main 메서드부터 시작해 쌓이면서 메서드가 실행되며, 순서대로 참조형 변수의 주소와 메서드는 stack 영역에 쌓이게 되고 그 과정에 필요한 변수는 heap 영역에 저장된다.  
...
11. ```public``` ===> 접근제어자, 아무 패키지에서나 접근 가능

12. ```void``` ===> 반환값이 없음을 의미한다(주로 함수 등)
...
4. 다형성(Polymorphism) - 하나의 이름으로 다양한 형태 사용 가능(메서드 오버라이딩)  
